...
# Endpoint 2: Read finetune status JSON and send last updated date
@app.get("/finetune-status")
async def finetune_status(status_json: str = 'Finetune_data/finetune_status.json'):
    """
    Reads the finetune status JSON and returns the last updated date.
    """
    try:
        with open(status_json, 'r') as f:
            status = json.load(f)
        last_updated = status.get('last_finetune', None)
        return {'last_finetune': last_updated}
    except Exception as e:
        return {'error': str(e)}

# New endpoint: fetch images from PostgreSQL database
from fastapi import Form

@app.post("/fetch-images")
async def fetch_images(
    image_type: str = Form(..., description="Type of images: normal or faulty"),
    limit: int = Form(10, description="Number of images to fetch"),
    temp_data_dir: str = Form("Finetune_data/temp_data", description="Target temp_data folder")
):
    """
    Fetch images from PostgreSQL database and store in temp_data folder.
    Assumes images are stored as file paths in the DB.
    """
    # Database config from application.yml
    db_config = {
        "host": "dpg-d3gb8du3jp1c73ek2cl0-a.singapore-postgres.render.com",
        "port": 5432,
        "database": "transformer_inspection_db",
        "user": "transformer_inspection_db_user",
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT image_path FROM images WHERE type=%s LIMIT %s", (image_type, limit)
        )
        rows = cursor.fetchall()
        target_folder = Path(temp_data_dir) / image_type
        ...
        cursor.close()
        conn.close()
        return {"status": "success", "fetched": saved, "target_folder": str(target_folder)}
    except Exception as e:
